...
def save_sparse_matrix_as_csv(tf_idf_matrix, vectorizer, output_file):
    """
    Saves the TF-IDF sparse matrix to a CSV file with three columns: document, word, and tf-idf value.
...
def main():
    ...
    base_name = '.'.join(input_file.split('.')[:-1])  # Handles filenames with multiple dots

    # Define output file names dynamically
    sparse_csv_file = f"{base_name}_tf_idf_sparse.csv"
    tf_idf_npz_file = f"{base_name}_tf_idf_sparse_matrix.npz"
    vocab_file = f"{base_name}_vocabulary.csv"
    # label_file = f"{base_name}_document_labels.csv"
